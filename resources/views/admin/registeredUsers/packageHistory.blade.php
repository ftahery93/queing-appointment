...
<!-- Modal 3 (Owner Details)-->
<!-- Modal -->
<div class="modal fade" id="myModal2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog"   style="width: 25%;">
        <div class="modal-content"  id="ownerDetails">
...
jQuery(document).ready(function ($) {
    ...
    $table4.DataTable({
        ...
        "ajax": {
            "type": "GET",
            url: "{{ url('admin/registeredUsers')}}/"+ID + '/packageHistory',
            complete: function () {
                $('.loading-image').hide();
            }
        },
        columns: [
            ...
            {data: 4, name: 'num_points', class: 'text-center'},
            {data: 5, name: 'num_days', class: 'text-center'},
            {data: 7, name: 'action', class: 'text-center'}
        ],
        order: [[3, 'desc']],
           "fnDrawCallback": function (oSettings) {
                                ...
                                $('.package_details').on('click', function (e) {
                                    ...
                                    $.ajax({
                                        type: "GET",
                                        async: true,
                                        url: "{{ url('admin/registeredUsers')}}/"+payment_id + '/packagePayment',
                                ...
                                $('.owner').on('click', function (e) {
                                    e.preventDefault();
                                    $('#ownerDetails').html('');
                                    var owner_id = $(this).attr('data-val');
                                    $('.loading-image').show();
                                    $.ajax({
                                        type: "GET",
                                        async: true,
                                        url: "{{ url('admin/registeredUsers')}}/"+owner_id + '/ownerDetail',
                                        success: function (data) {
                                            $('#ownerDetails').html(data.html);
                                        },
                                        complete: function () {
                                            $('.loading-image').hide();
                                        }
