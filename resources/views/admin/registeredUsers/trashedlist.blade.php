...
jQuery(document).ready(function ($) {
    ...
    $table4.DataTable({
        ...
        "ajax": {
            "type": "GET",
            "url": '{{ url("admin/registeredUsers/trashedlist") }}',
            complete: function () {
                $('.loading-image').hide();
            }
        },
            ...
            $('.restore').on('click', function (e) {
                ...
                if (confirm("Are you sure?")) {
                    ...
                    $.ajax({
                        type: "POST",
                        async: true,
                        url: "{{ url('admin/registeredUsers/trashed')}}/"+ID + '/restore',
            ...
            $('.delete').on('click', function (e) {
                ...
                if (confirm("Are you sure?")) {
                    ...
                    $.ajax({
                        type: "POST",
                        async: true,
                        url: "{{ url('admin/registeredUsers/trashed')}}/"+ID + '/delete',
                        data: {id: ID, _token: '{{ csrf_token() }}'},
                        success: function (data) {
                            $table4.DataTable().ajax.reload(null, false);
                            toastr.success(data.response, "", opts);
