...
<li>
    ...
    <a href="{{ url("admin/".$VendorBranch->vendor_id."/vendorBranches")  }}">{{ $VendorBranch->name_en }}</a>
</li>
@endsection

@section('pageheading')
Vendors - {{ $VendorBranch->name_en }}
@endsection
                  ...
                  <div class="col-md-6">
                    <div class="dropzone-file-section mt-1">
                        <div class="table table-striped files uploadedFiles" style="margin-bottom: 0">
                            ...
                            <div class="file-row f{{ $vendorImage->id }}" id="vendor_images">                                                       
                                <div class="column">
                                    <span class="preview">
                                        <img src="{{ url('public/vendors_images/'.$vendorImage->image) }}" style="width:40px;height:40px;">                                               
                                    </span>
                                </div>

                                <div class="column">

                                    <span class="btn btn-danger btn-xs pull-right delete_afterpageload" id="deleteFile" data-id="{{ $vendorImage->id }}" ><i class="fa fa-trash-o"></i> Delete</span>
...
$(document).ready(function () {
    ...
    var myDropzone = new Dropzone(".uploadProposal", {// Make the whole body a dropzone
        url: "{{ url('admin/vendorBranches/'.$branchID.'/images') }}", // Set the url
        thumbnailWidth: 40,
        thumbnailHeight: 40,
        parallelUploads: 1,
                ...
                removeButton.addEventListener("click", function (e) {
                    if (file.size > 1048576) {
                        ...
                        $.ajax({
                            type: "POST",
                            async: true,
                            url: "{{ url('admin/vendorBranches/deleteImage')}}/" + file.id,
...
$(document).on('click', '.delete_afterpageload', function (e) {
    if (confirm('{{ config('global.deleteConfirmation') }}')) {
        ...
        $.ajax({
            type: "POST",
            async: true,
            url: "{{ url('admin/vendorBranches/deleteImage')}}/" + ID,
