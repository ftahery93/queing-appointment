...
@section('title')
{{ ucfirst($vendorName->name) }} - Pending  Class Commission 
@endsection
                ...
                <table class="table table-bordered datatable" id="table-4">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center col-sm-3"> Previous Commission (%)</th>
                            <th class="text-center col-sm-2"> Commission (%)</th>
                        </tr>
                    </thead>
...
<script type="text/javascript">
                        jQuery(document).ready(function ($) {
                            ...
                            $table4.DataTable({
                                ...
                                "ajax": {
                                    "type": "GET",
                                    "url": '{{ url("admin/pendingCommission/$vendor_id") }}',
                                    complete: function () {
                                        $('.loading-image').hide();
                                    }
                                },
                                columns: [
                                    ...
                                    {data: 1, name: 'name_en'},
                                    {data: 2, name: 'temp_price', orderable: false, class: 'text-center'},
                                    {data: 3, name: 'commission_perc', orderable: false, class: 'text-center'},
                                    {data: 4, name: 'temp_commission_perc', orderable: false, class: 'text-center'},
                                ],
                                order: [[0, 'desc']],
...
// Removed the checked state from "All" if any checkbox is unchecked
     ...
     /*---Commission column click event--*/
         $(document).on('click', '.commission_field', function () { 
             $(this).removeClass('commission_field');
          $(this).html('<input type="tel" class="form-control number_only text-center temp_commission_perc" autocomplete="off"  value="" name="temp_commission_perc" >'); 
        });
    ...
    function ConfirmDelete() {
        ...
        if (confirm('{{ config('global.deleteConfirmation') }}')) {
            ...
            $.ajax({
                type: "POST",
                async: true,
                "url": '{{ url("admin/pendingClasses/addCommission") }}',
