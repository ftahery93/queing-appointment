...
<li>
    ...
    <a href="{{ url($configM1.'/subscribers')  }}">Subscribed</a>
</li>
<li>

    <a href="{{ url($configM1.'/archivedSubscribers')  }}">Archived</a>
</li>
@endsection
...
jQuery(document).ready(function ($) {
    ...
    $table4.DataTable({
        ...
        "ajax": {
            "type": "GET",
            "url": '{{ url("$configM1")}}/'+ID + '/packageHistory',
            complete: function () {
                $('.loading-image').hide();
            }
        },
        columns: [
            {data: 0, name: 'name_en'},
            {data: 1, name: 'price', class: 'text-center', orderable: false},
            {data: 2, name: 'start_date', class: 'text-center', orderable: false},
            {data: 3, name: 'end_date', class: 'text-center'},
            {data: 4, name: 'num_days', class: 'text-center', orderable: false},
            {data: 6, name: 'action', class: 'text-center', orderable: false}
        ],
        order: [[3, 'desc']],
           "fnDrawCallback": function (oSettings) {
                                ...
                                $('.package_details').on('click', function (e) {
                                    ...
                                    $.ajax({
                                        type: "GET",
                                        async: true,
                                        "url": '{{ url("$configM1")}}/'+payment_id + '/packagePayment',
