...
$(document).ready(function () {
    ...
    var myDropzone = new Dropzone(".uploadProposal", {// Make the whole body a dropzone
        "url":  '{{ url("$configName/vendorBranches/$branchID/images") }}',  // Set the url
        thumbnailWidth: 40,
        thumbnailHeight: 40,
        parallelUploads: 1,
                ...
                removeButton.addEventListener("click", function (e) {
                    if (file.size > 1048576) {
                        ...
                        $.ajax({
                            type: "POST",
                            async: true,
                            "url":  '{{ url("$configName/vendorBranches/deleteImage") }}/' + file.id, 
...
$(document).on('click', '.delete_afterpageload', function (e) {
    if (confirm('{{ config('global.deleteConfirmation') }}')) {
        ...
        $.ajax({
            type: "POST",
            async: true,
            "url":  '{{ url("$configName/vendorBranches/deleteImage") }}/' + ID, 
